...
export interface IBufferPool {
  ...
  /** 
   * Return the current dirty page table for checkpoints.
   * Maps pageId -> recLsn (LSN of the first log record that dirtied the page).
   */
  getDirtyPageTable(): Map<PageId, LSN>;
}
...
export interface CatalogEntry {
  ...
  heapFile:            any; // IHeapFile instance injected at runtime
  readonly schema:     Schema;
  readonly primaryKey: string;
  readonly indexes:    Record<IndexId, any>; // index info with IBPlusTree injected at runtime
...
/**
 * TupleSlot abstraction separating the physical tuple from its metadata.
 */
...
export interface IOperator {
  ...
  /**
   * Return the next TupleSlot, or null when exhausted.
