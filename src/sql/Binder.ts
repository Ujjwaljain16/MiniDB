...
export class Binder {
  ...
  private bindInsert(stmt: InsertStmt): LogicalInsert {
    ...
    // In our AST, if no columns, maybe columns array is empty. For now assume explicitly provided.
    
    const boundValues: BoundExpression[][] = [];
    for (const row of stmt.values) {
      if (row.length !== stmt.columns.length) {
        throw new BindError(`INSERT has ${row.length} values but ${stmt.columns.length} columns`);
      }
