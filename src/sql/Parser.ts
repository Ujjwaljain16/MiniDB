...
function transformInsert(stmt: CstNode): InsertStmt {
  ...
  const columns = insertClause!.columns!.expr!.items!.map((i: CstNode) => i.name! as string);
...
function transformDelete(stmt: CstNode): DeleteStmt {
  ...
  const table = (deleteClause.table as CstNode).name as string;
  const where = whereClause ? transformExpr(whereClause.expr!) : undefined;
...
function transformExplain(stmt: CstNode): ExplainStmt {
  ...
  if (stmt.explainKw && (stmt.explainKw.name as CstNode)?.text?.toUpperCase() === 'EXPLAIN' && 
      stmt.queryPlanKw && (stmt.queryPlanKw.name as CstNode)?.text?.toUpperCase() === 'ANALYZE') {
