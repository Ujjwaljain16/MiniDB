...
describe('LOCK_COMPAT matrix', () => {
  ...
  const cases: Array<[LockMode, LockMode, boolean]> = [
    [LockMode.IS, LockMode.IS, true],  [LockMode.IS, LockMode.IX, true],
    [LockMode.IS, LockMode.S,  true],  [LockMode.IS, LockMode.X,  false],
    [LockMode.IX, LockMode.IS, true],  [LockMode.IX, LockMode.IX, true],
    [LockMode.IX, LockMode.S,  false], [LockMode.IX, LockMode.X,  false],
    [LockMode.S,  LockMode.IS, true],  [LockMode.S,  LockMode.IX, false],
    [LockMode.S,  LockMode.S,  true],  [LockMode.S,  LockMode.X,  false],
    [LockMode.X,  LockMode.IS, false], [LockMode.X,  LockMode.IX, false],
  ...
  test('matrix is 4×4', () => {
    expect(LOCK_COMPAT.length).toBe(4);
    for (const row of LOCK_COMPAT) {
      expect(row.length).toBe(4);
    }
  });

  test('IS=0, IX=1, S=2, X=3 numeric values are stable', () => {
    expect(LockMode.IS).toBe(0);
    expect(LockMode.IX).toBe(1);
    expect(LockMode.S).toBe(2);
    expect(LockMode.X).toBe(3);
  });
});
